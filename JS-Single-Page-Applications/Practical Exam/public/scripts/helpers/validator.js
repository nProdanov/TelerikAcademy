...
(function () {
    ...
    function checkIfUsernameOnlyAllowedSymbols(text) {
        ...
        if (!pattern.test(text)) {
            throw new Error('Username must contains only lower or upper case symbols,digits,  "." or "_"');
        }
    }

    function checkIfPasswordOnlyAllowedSymbols(text){
        let pattern = /^[a-zA-Z0-9]+/;
        if (!pattern.test(text)) {
            throw new Error('Password must contains only lower or upper case symbols or digits');
        }
    }
    ...
    class Validator {
        ...
        validatePassword(password){
            ...
            checkIfTextIsWithCorrectLength(password, 'Password');
            checkIfPasswordOnlyAllowedSymbols(password);
        }
