using System;
using System.Collections.Generic;

using InheritanceAndPolymorphism.Courses;

namespace InheritanceAndPolymorphism
{
    public class CoursesExamples
    {
        public static void Main()
        {
            var separator = new string('-', 20);

            LocalCourse localCourse = new LocalCourse("Databases");
            Console.WriteLine(localCourse);

            localCourse.Lab = "Enterprise";
            Console.WriteLine(localCourse);

            localCourse.Students = new List<string>() { "Peter", "Maria" };
            Console.WriteLine(localCourse);

            localCourse.TeacherName = "Svetlin Nakov";
            localCourse.Students.Add("Milena");
            localCourse.Students.Add("Todor");
            Console.WriteLine(localCourse); // do not appear because of encapsulation

            Console.WriteLine(new string('-', 20));

            localCourse.AddStudent("Milena");
            localCourse.AddStudent("Todor");
            Console.WriteLine(localCourse); // now appear because added through the interface

            Console.WriteLine(separator);

            OffsiteCourse offsiteCourse = new OffsiteCourse(
                "PHP and WordPress Development", 
                "Mario Peshev", 
                new List<string>() { "Thomas", "Ani", "Steve" });

            Console.WriteLine(offsiteCourse);

            offsiteCourse.Town = "Nova Zagora";
            Console.WriteLine(offsiteCourse);
        }
    }
}
